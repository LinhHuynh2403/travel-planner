...
export function TodayTab({ itinerary, memories, userName, onSeeWholeDay, onOpenChat }: TodayTabProps) {
  ...
  let weatherAdvisory = 'Check the Packing tab for a full week-ahead forecast.';
  if (todayWeather) {
    ...
    if (todayRainy) {
      weatherAdvisory = `Bring a rain shell today — ${todayWeather.note.toLowerCase()}.`;
    } else if (tomorrowRainy) {
      weatherAdvisory = 'No rain today — but pack the rain shell tomorrow.';
    } else {
      weatherAdvisory = 'Nice weather for exploring today.';
    }
  }
  ...
  return (
    <div>
      ...
      {memories.length > 0 && (
        <div className="flex flex-wrap gap-2 items-center mt-3.5 mb-1">
          <span className="text-[15px] font-extrabold text-jz-soft flex items-center gap-1.5">
            <Heart className="w-[15px] h-[15px]" fill="#F0A742" stroke="#F0A742" /> JourZy remembers:
          </span>
          {memories.map(m => (
      ...
      {todayDay && (
        <>
          <SectionTitle>Today in {cityName} — Day {todayDay.dayNumber} of {days.length}</SectionTitle>
            ...
            <div className="flex items-center gap-3.5">
              ...
              <div>
                {todayWeather ? (
                  ...
                  <p className="text-2xl font-black text-jz-ink m-0">{cityName} weather</p>
                )}
            ...
            <>
              ...
              <Card>
                <p className="text-[15.5px] font-extrabold text-jz-gold uppercase tracking-wide">
                  {upNext.activity.time} · {upNext.activity.category === 'food' ? 'Meal' : upNext.activity.category}
                </p>
                ...
                {nextNext?.travelTimeFromPrevious && (() => {
                  ...
                  return (
                    <div className="flex items-center gap-2 mt-3 text-jz-soft text-[16.5px] font-bold">
                      <TravelIcon className="w-5 h-5" /> Then: {nextNext.travelTimeFromPrevious} to {nextNext.title}
                    </div>
                  );
                })()}
          ...
          <div className="grid gap-3 mt-4">
            <BigButton icon={MapIcon} onClick={() => onSeeWholeDay(todayDay.dayNumber)}>See my whole day</BigButton>
            <BigButton icon={MessageCircle} secondary onClick={onOpenChat}>Talk to JourZy</BigButton>
          </div>

          {emergency && (
            <Card tint="bg-jz-goldTint" className="mt-4 border-none">
              <p className="text-[16.5px] leading-relaxed font-bold text-jz-goldSoft">
                <Phone className="w-[17px] h-[17px] inline -mt-0.5 mr-1" /> Good to know: in {cityName}, dial <b className="text-jz-goldInk">{emergency.police}</b> for police
                {emergency.ambulance && <> and <b className="text-jz-goldInk">{emergency.ambulance}</b> for an ambulance</>}. It's all in your Tips tab — just a friendly heads-up.
              </p>
            </Card>
          )}
        </>
      ...
      {!todayDay && (
        <p className="mt-6 text-jz-body text-jz-soft font-bold">Your trip schedule will appear here once JourZy builds it. ✨</p>
      )}
    </div>
  );
}
